...
public class FrostedSidebar: UIViewController {
    ...
    public init(itemImages: [UIImage], colors: [UIColor]?, selectedItemIndices: NSIndexSet?){
        ...
        //(selectedIndices = selectedItemIndices ? NSMutableIndexSet(indexSet: selectedItemIndices!) : NSMutableIndexSet() != nil)
        borderColors = colors
        images = itemImages
...
//        
    ...
    public func showInViewController(viewController: UIViewController, animated: Bool){
        ...
        if animated{
            UIView.animateWithDuration(NSTimeInterval(animationDuration), delay: 0, options: UIViewAnimationOptions(rawValue: UInt(kNilOptions))!, animations: animations, completion: completion)
        } else{
            animations()
            completion(true)
        }
    ...
    private class CalloutItem: UIView{
        ...
        override func touchesBegan(touches: NSSet!, withEvent event: UIEvent!) {
            ...
            if (originalBackgroundColor?.getRed(&r, green: &g, blue: &b, alpha: &a) != nil){
                darkerColor = UIColor(red: max(r - darkenFactor, 0), green: max(g - darkenFactor, 0), blue: max(b - darkenFactor, 0), alpha: a)
            } else if (originalBackgroundColor?.getWhite(&r, alpha: &a) != nil){
